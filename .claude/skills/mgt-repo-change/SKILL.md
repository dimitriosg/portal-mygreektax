...
- [x] Audit every call site of `resolve_case_for_inbound`. Report only: file, line, exact parameter set, and any conditional path that can drop a discriminator key at runtime. No code changes. Reported 02 Aug 2026, two call sites, both currently selecting the current version. No PR, the deliverable was the report.
