...
- [x] `brain-mygreektax` `.old` cleanup: `index.js.old` at root, five `src/index.js.backup*.old`, `wiki/index_old.md`, `wiki/rules/OLD BACKUP/`, `pricing/price-table_old.md`. PR only, never merge. Brain PR #5, 9 files removed, merged 02 Aug 2026 on Jim's explicit instruction in session, overriding the never-merge default; deploy run 54 green. The audit also produced brain PR #6 (adds `src/package-lock.json`, since caret ranges with no lockfile meant every deploy re-resolved dependencies) and brain PR #7 (removes the partner's licence number, AFM and ΔΟΥ from the public `pricing/price-table.md`).
