...
function Page() {
  ...
  const decide = useMutation({
    ...
    onSuccess: () => { toast.success("Decision recorded"); qc.invalidateQueries({ queryKey: ["change-requests"] }); },
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:py-8 space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Partner-submitted job changes awaiting your review.</p>
        </div>
                ...
                {requests.map((r) => (
                  <tr key={r.id} className="border-t border-border align-top">
                    <td className="px-3 py-2">
                      <Link to="/jobs/$jobId" params={{ jobId: r.airtable_job_id }} className="font-medium hover:underline">
                    ...
                    <td className="px-3 py-2">{(r.current_value || "—") + " → " + (r.requested_value || "—")}{r.reason && <div className="text-xs text-muted-foreground mt-1">Reason: {r.reason}</div>}</td>
                    <td className="px-3 py-2">{r.status}</td>
                    <td className="px-3 py-2 whitespace-nowrap text-xs text-muted-foreground">{formatDateTime(r.created_at)}</td>
                    <td className="px-3 py-2">
                      {r.status === "pending" && (
                        <div className="flex gap-2">
                          <Button size="sm" onClick={() => decide.mutate({ id: r.id, decision: "approved" })} disabled={decide.isPending}>Approve</Button>
                          <Button size="sm" variant="outline" onClick={() => decide.mutate({ id: r.id, decision: "rejected" })} disabled={decide.isPending}>Reject</Button>
                        </div>
                      )}
                    </td>
                  </tr>
