...
// Used when the caller sends no subject (blank field on the desk). Must match
// the placeholder in AiReviewDesk.tsx (DEFAULT_SUBJECT). Change both together.
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Optional. Blank/absent -> the default subject is used.
