...
export const getAppendix = createServerFn({ method: "GET" })
  ...
  .inputValidator((d?: { partnerUserId?: string }) =>
    z.object({ partnerUserId: z.string().uuid().optional() }).parse(d ?? {}),
  )
  .handler(async ({ data, context }) => {
    ...
    if (isAdmin) {
      ...
      if (!selectedPartnerId) {
        return { isAdmin, partners, selectedPartnerId: null, partnerName: null, rows: [] };
      }
    }
