...
export const AiReviewDesk: React.FC<AiReviewDeskProps> = ({ jobId }) => {
  ...
  useEffect(() => {
    ...
    const fetchDraft = async () => {
      ...
      // Signature always resets to the default on load / regenerate. Subject
      // starts blank so the placeholder default applies unless it's typed.
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 border rounded-xl bg-slate-50/50 shadow-inner animate-in fade-in slide-in-from-top-4 duration-300">
      ...
      {/* RIGHT: editable fields (subject + message body + signature). */}
      <Card className="border border-slate-200 bg-white shadow-md flex flex-col">
        ...
        <CardContent className="flex-1 flex flex-col gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mb-1">Leave blank to use the default.</p>
            <input
              type="text"
              value={subject}
