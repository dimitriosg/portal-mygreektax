...
--
-- WHY THIS IS A DROP AND RECREATE, NOT create or replace.
--
-- `create or replace function` matches on name AND argument list. Adding
-- parameters would therefore create a SECOND confirm_payment alongside the
-- existing one, and Postgres would choose between them by call shape. That is
-- exactly the failure already in this project's history: two overloads of
-- resolve_case_for_inbound, one of which silently bypassed deduplication. So
-- the old signature is dropped explicitly, in the same transaction.
--
