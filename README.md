...
Everything runs as a single TanStack Start application deployed to a Cloudflare Worker. Supabase is the system of record for cases, leads, clients, drafts, auth, and delivery logs. Airtable is retained only for the legacy partner job data. The Brain lives in the separate `brain-mygreektax` repo and is reached over HTTPS.
...
| Auth and data  | Supabase (`@supabase/supabase-js`), the primary store                     |
| Rich text      | TipTap v3 (starter kit, link, underline, color, text style) with DOMPurify |
| Drag and drop  | @dnd-kit (partner job ordering)                                            |
| Email          | React Email templates, Mailgun send path, Lovable email and webhook helpers |
| Charts         | Recharts (admin analytics)                                                 |
| Validation     | Zod                                                                        |
| Legacy data    | Airtable (partner job records only, being superseded by Supabase)         |
| Analytics      | Plausible                                                                  |
...
src/
  ...
  lib/
    ...
    airtable.server.ts  airtable-shared.ts   (legacy)
...
- Airtable base credentials, only if working on the legacy job tracking layer
...
The keys documented in `.env.example`:
...
AIRTABLE_TABLE_MESSAGES=your-airtable-messages-table-id
...
- The Airtable keys are only needed for the legacy job tracking layer.
...
- **Airtable is legacy.** New work targets Supabase. Airtable remains only for partner job records during the transition.
