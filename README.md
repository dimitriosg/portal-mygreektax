...
- `N8N_GMAIL_SYNC_URL` and `N8N_GMAIL_SYNC_SECRET` drive the Refresh button on the Correspondence view of `/leads`. The URL is the **Portal Refresh Requested** webhook on n8n workflow `uSQOKDb9YLNxiIIT` ("20 · Sync Gmail to messages"); the secret is sent as `X-Mgt-Portal-Secret` and must equal the value in that workflow's header-auth credential. **Both are unset today**, so the button renders disabled with the reason on it rather than silently doing nothing. Set them in Cloudflare, never here — and note the webhook only answers once the workflow is activated, which also starts its 2-hourly schedule.
